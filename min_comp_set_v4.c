...
/*
	...
	consider classify the functions to some equal classes,
	each class has the same one-function generating result 
...
typedef struct{
	int results; // 16-bit number, indicate if 0000, 00001, ..., 1111 is existed
	int len;
	int * inds;
} EqualClass;
...
int check_complete(FuncTab* cands, int cands_len, int* chooses, int chooses_len, int* class_val){
    int i,j,k,l,m,n;
    int flag[16];
    int comp[16];
    int res_ind[4];
...
void get_one_complete_set_and_delete(FuncTab* cands, int* cands_len){
    ...
    for(i = 0; i < (*cands_len); i++){
        p[0] = i;
        if(check_complete(cands, (*cands_len), p, 1, &class_val)){
            flag[cnt++] = i;
...
int get_complete_C_n_m(int n, int m, Complete* complete, int complete_len, FuncTab* cands, int cands_len, int need_compute){
    int* p;
    int i, j;
	int ind, is_complete, origin_complete_len,class_val;
    int * complete_flag;
    double cnm;
	long long cnt = 0;
    ...
    while(1){
        if(p[ind] >= n){
            ...
            if(need_compute)
                is_complete = check_complete(cands, cands_len, p, m, &class_val);	
            else
                is_complete = 0;
			if(is_complete){
...
void get_equal_class(FuncTab* cands, int cands_len, EqualClass * equal_class, int* equal_class_len){
    int i, j, k, tmp, cnt, class_val;
    short cls_flag[65536];
    int cad_flag[300];
    ...
    for(i = 0; i < cands_len; i++){
    	tmp = check_complete(cands, cands_len, &i, 1, &class_val);
    	assert(tmp == 0);
    	cls_flag[class_val]++;
    	cad_flag[i] = class_val;
    }
    
    cnt = 0;
    for(i = 0; i < 65536; i++){
    	if(cls_flag[i] > 0){
    		equal_class[cnt].results = i;
    		equal_class[cnt].len = cls_flag[i];
    		equal_class[cnt].inds = (int*)malloc(sizeof(int)*cls_flag[i]);
    		k = 0;
    		for(j = 0; j < cands_len; j++){
    			if(cad_flag[j] == i){
    				equal_class[cnt].inds[k++] = j;
    			}
    		}
    		cnt++;
    	}
...
void print_equal_class(EqualClass* equal_class, int equal_class_len, FuncTab* cands){
	int i,j,k,tmp;
	int res1[16];
	int res2[4];
	for(i = 0; i < equal_class_len; i++){
		tmp = equal_class[i].results;
		printf("Class[%d]: cnt:%d results:%d\n", i + 1, equal_class[i].len, tmp);
		to_base_n(tmp, 2, 16, res1);
		for(j = 0; j < 16 ; j++){
			if(res1[j] >= 1){
				to_base_n(15-j, 2, 4, res2);
				for(k = 0; k < 4; k++){
					printf("%d", res2[k]);
				}
				printf("\n");
		...
		for(j = 0; j < equal_class[i].len; j++){
			tmp = equal_class[i].inds[j];
			printf("%s %d ", cands[tmp].ident, cands[tmp].args_count);
			for(k = 0; k < cands[tmp].bits_count; k++)
				printf("%d",cands[tmp].bits[k]);
			printf(" ");
...
void check_equal_class(EqualClass* equal_class, int equal_class_len, FuncTab* cands, int cands_len){
	int i,j;
	int p[2];
	int x;
	int y;
	...
	for(x = 0; x < equal_class_len; x++){
		for(y = x + 1; y < equal_class_len; y++){
			cur = 0;
			for(i = 0; i < equal_class[x].len; i++){
				p[0] = equal_class[x].inds[i];
				for(j = 0; j < equal_class[y].len; j++){
					p[1] = equal_class[y].inds[j];
					tmp = check_complete(cands, cands_len, p, 2, &class_val);
					printf("x=%d y=%d i=%d j=%d class val: %d\n", x+1, y+1, i+1, j+1, class_val);
					if(cur == 0)
						cur = class_val;
					else
						assert(cur == class_val);
				}
			}
		}
	}
...
int main(){
	...
	// classify the rest functions to some equal class
	get_equal_class(cands, cands_len, equal_class, &equal_class_len);
	
	print_equal_class(equal_class, equal_class_len, cands);

	check_equal_class(equal_class, equal_class_len, cands, cands_len);

	// C(n,2)~C(n,n-1)
	/*
	for(i = 2; i < cands_len; i++){		
		printf("Choose %d from %d %d\n", i, cands_len, complete_len);
